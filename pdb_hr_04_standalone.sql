...
where dbid in (db_id('TalentGate'),db_id('EngineeringDB'),db_id('MarketingDB'),db_id('SupportDB'),db_id('TalentGlobalDB'),db_id('TalentCommonDB'),db_id('TalentSalaryDB')) and cmd = 'AWAITING COMMAND' and spid <> @@spid;
exec(@sql);
end;
go
...
if db_id('TalentSalaryDB') 	is not null drop database TalentSalaryDB;
...
create database TalentSalaryDB;
use PdbLogic;
...
exec PdbcreatePartition 'TalentGate','TalentSalaryDB',@DatabaseTypeId=4,@StandAloneSchemaName='Salary';
